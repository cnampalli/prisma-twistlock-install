...
| `internal_backup_url` (default), `prisma_restore_api_user` (default) | `survey_ops_backup.yml`, `survey_dr_drill.yml` | real non-secret values |

## Secrets

Secrets are **never committed**. Only four are actually consumed by the
playbooks: `prisma_backup_token`, `internal_backup_url` (backup, by the
`prisma-30-ops` job) and `prisma_restore_api_user`, `prisma_restore_api_password`
(DR restore, by `prisma-40-dr-drill`).

**Default — job-template surveys (no superuser needed).** Both templates are
operator-launched, so the four vars are supplied via **surveys**
(`survey_ops_backup.yml`, `survey_dr_drill.yml`): secret fields are `password`
type — masked in the UI, encrypted at rest, injected as `extra_vars` at launch.
This is the default because creating credential *types* requires a Controller
System Administrator, which the install team typically does not have.

**Optional — Path A: custom credential types** (`controller_credential_types.yml`,
key currently `controller_credential_types_optional` so it is **not** applied).
If you have superuser (or hand the spec to the platform team), rename the key
back, restore the `prisma-backup-prod` / `prisma-restore-api-prod` credentials in
`controller_credentials.yml`, attach them to the job templates, and drop those
templates' backup/restore surveys. The injectors push each secret as an
`extra_var` at launch, masked in output.

The Controller API password / token for `configure.yml` itself comes from the
`CONTROLLER_*` environment variables above.

The licence key + LDAPS bind password are **not** handled here under the default
(survey) path — no playbook reads them; they are entered in the Console UI during
the manual Phase 11 first-run wizard. (The optional `Prisma First-Run` credential
type exists only to keep them in AAP if you adopt Path A.)
...
- **Site** scoping uses the launch-time **Limit** field
  ...
  inventory groups. Surveys carry in-playbook variables **and** the backup/DR
  secrets: `survey_ops_backup.yml` (backup token + URL on `prisma-30-ops`) and
  `survey_dr_drill.yml` (DR behaviour + restore API creds on `prisma-40-dr-drill`).
